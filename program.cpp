...
void program::insert(int lnum,QString newtext)
{
        ...
        if(state == 0)      //替换
        {
            ...
            prolength++;
            tmp->textline = prolength;
        }
        else          //添加
        {
            statement *tmp = move(lnum);
            statement *tmpq = tmp;
            if(tmp->next == nullptr)             //尾部添加statement
            {
               prolength++;
               tmp->next = new statement(newtext,nullptr,prolength);
            }
            else{
                prolength++;
                tmp->next = new statement(newtext,tmpq->next,prolength);
            }
       }
}
